...
class WebGraph():
    ...
    def query_score(self, query=''):
        ...
        for entry in res:
            ...
            occurances = 0
            terms = []
            url = self._index_to_url(iteration)
            iteration += 1

            if (len(query) != 0):
                for word in query.split():
                    terms.append([str(word), float(1)])
                    temp = vectors.most_similar(query)
                    for i in range(5):
                        terms.append([str(temp[i][0]), float(temp[i][1])])
...
if __name__=='__main__':
    ...
    query_score = g.query_score(query=args.search_query)
